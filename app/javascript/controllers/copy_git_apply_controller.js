...
export default class extends Controller {
  ...
  async copy(event) {
    ...
    try {
      ...
      if (!diff || diff.trim() === '') {
        this.showFeedback(event.target, 'No changes to apply', false)
        return
      }
      
      const command = `(cd "$(git rev-parse --show-toplevel)" && git apply --3way <<'EOF'\n${diff}\nEOF\n)`
      
      await navigator.clipboard.writeText(command)
      this.showFeedback(event.target, 'Copied!', true)
      
    } catch (error) {
      console.error('Error copying git apply command:', error)
