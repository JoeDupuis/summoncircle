...
class SecretTest < ActiveSupport::TestCase
  test "validates presence of key" do
    secret = Secret.new(secretable: projects(:one), value: "secret_value")
    assert_not secret.valid?
    assert_includes secret.errors[:key], "can't be blank"
  end

  test "validates presence of value" do
    secret = Secret.new(secretable: projects(:one), key: "API_KEY")
    assert_not secret.valid?
  ...
  test "validates uniqueness of key within project" do
    ...
    Secret.create!(secretable: project, key: "API_KEY", value: "secret1")

    duplicate_secret = Secret.new(secretable: project, key: "API_KEY", value: "secret2")
  ...
  test "allows same key in different projects" do
    ...
    Secret.create!(secretable: project1, key: "API_KEY", value: "secret1")
    secret2 = Secret.new(secretable: project2, key: "API_KEY", value: "secret2")

    assert secret2.valid?
  end

  test "encrypts value" do
    secret = Secret.create!(secretable: projects(:one), key: "API_KEY", value: "secret_value")
